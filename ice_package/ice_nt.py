...
class INT(WaterAlgorithm):
    ...
    def __init__(self, n=2, m=4, O_O_distance=2.71, O_H_distance=1.0, intermediate_saves=[], group_saves=[], options = None):
        WaterAlgorithm()
        self.n = n # NUMBER OF MOLECULES IN A LAYER
        self.m = m # LAYER COUNT
        folder = "INT_%i_%i" % (n, m)
        self.initialize(O_H_distance=O_H_distance, O_O_distance=O_O_distance, intermediate_saves=intermediate_saves, group_saves=group_saves, folder=folder, options = options)
